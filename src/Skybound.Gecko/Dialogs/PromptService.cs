...
namespace Skybound.Gecko
{
	...
	class PromptService : nsIPromptService, nsIPromptService2, nsINonBlockingAlertService
	{
		...
		{
            using (ConfirmDialog dialog = new ConfirmDialog(aText, aDialogTitle, "OK", null, null, aCheckMsg))
            {
                dialog.ShowDialog();
		...
		{
            using (PromptDialog dialog = new PromptDialog(aDialogTitle, aText, aValue, aCheckMsg))
            {
                DialogResult result = dialog.ShowDialog();
		...
		public bool PromptPassword(nsIDOMWindow aParent, string aDialogTitle, string aText, ref string aPassword, string aCheckMsg, IntPtr aCheckState)
		{
            ...
            using (PasswordDialog dialog = new PasswordDialog(aDialogTitle, aText, "", aPassword, aCheckMsg, checkState))
            {
