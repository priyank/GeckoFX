...
namespace Skybound.Gecko
{
	...
	public class GeckoNode
	{
		...
		public override bool Equals(object obj)
		{
			if (this == obj)
				...
				return GetHashCode() == (obj as GeckoNode).GetHashCode();
		...
		{
			IntPtr pUnk = Marshal.GetIUnknownForObject(_DomObject);
			try
			{
        ...
        public ushort NodeType { get { return _DomObject.GetNodeType(); } }
	...
	public class GeckoElement : GeckoNode
	{
	    ...
	    public string ElementValue
	    {
            get
            {
                if (String.Compare(TagName, "input", true) == 0)
                    return nsString.Get((DomElement as nsIDOMHTMLInputElement).GetValue);
                return InnerHtml;
            }
            set
            {
                if (String.Compare(TagName, "input", true) == 0)
                    nsString.Set((DomElement as nsIDOMHTMLInputElement).SetValue, value);
                else
                    InnerHtml = value;
            }
	    }
        ...
        public bool Click()
        {
            if (String.Compare(TagName, "input", true) == 0)
            {
                (DomElement as nsIDOMHTMLInputElement).click();
                return true;
            }
            if (String.Compare(TagName, "button", true) == 0)
            {
                (DomElement as nsIDOMNSHTMLButtonElement).click();
                return true;
            }
            return false;
        }
	    ...
	    ScrollAndClient_nsIDOMNSElement ScrollAndClientProvider
	    {
	        get
	        {
	            return (Xpcom.GeckoVersion >= 0x01090100) ? (ScrollAndClient_nsIDOMNSElement)DomNSElement : DomNSHTMLElement;
	        }
	    }

        public int ScrollLeft { get { return ScrollAndClientProvider.GetScrollLeft(); } set { ScrollAndClientProvider.SetScrollLeft(value); } }
        public int ScrollTop { get { return ScrollAndClientProvider.GetScrollTop(); } set { ScrollAndClientProvider.SetScrollTop(value); } }
        public int ScrollWidth { get { return ScrollAndClientProvider.GetScrollWidth(); } }
        public int ScrollHeight { get { return ScrollAndClientProvider.GetScrollHeight(); } }
        public int ClientWidth { get { return ScrollAndClientProvider.GetClientWidth(); } }
        public int ClientHeight { get { return ScrollAndClientProvider.GetClientHeight(); } }
		...
		{
			DomNSHTMLElement.ScrollIntoView(top ? 1 : 0);
		}
		...
		/// <summary>
		/// From Gecko 1.9: Returns a set of elements with the given class name .  This element and all child elements are searched.
		/// </summary>
		...
		/// <summary>
		/// From Gecko 1.9: Gets a rectangle which represents the union of all bounding rectangles within the element.
		/// </summary>
		...
		/// <summary>
		/// From Gecko 1.9: Returns an array containing all bounding rectangles within the element.
		/// </summary>
		/// <returns></returns>
	...
	public class GeckoDocument : GeckoNode
	{
		...
		public class StyleSheetCollection : IEnumerable<GeckoStyleSheet>
		{
			...
			{
				List = ((nsIDOMDocumentStyle)document.DomDocument).GetStyleSheets();
			}
        ...
        {
            DomDocument.Write(new nsAString(str));
        }
		
		/// <summary>
		/// From Gecko 1.9: Gets the currently focused element.
		/// </summary>
		...
		/// <summary>
        /// From Gecko 1.9: Returns a set of elements with the given class name. When called on the document object, the complete document is searched, including the root node.
		/// </summary>
		/// <param name="classes"></param>
		/// <returns></returns>
		public GeckoNodeCollection GetElementsByClassName(string classes)
		...
		/// <summary>
        /// From Gecko 1.9: Returns the element visible at the given point, relative to the upper-left-most visible point in the document.
		/// </summary>
		/// <param name="x"></param>
		/// <param name="y"></param>
		/// <returns></returns>
	...
	public class GeckoWindow
	{
		...
		{
			get { return _Selection ?? (_Selection = new GeckoSelection(_DomWindow.GetSelection())); }
		}
