		...
		public object this[string prefName]
		{
			...
			set
			{
				...
				if (existingType != 0 && existingType != assignedType)
                    throw new InvalidCastException(String.Format("A {0} value may not be assigned to '{1}' because it is already defined as {2}.", value.GetType().Name, prefName, GetPreferenceType(prefName).Name));
